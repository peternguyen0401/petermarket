<?php
   ...
   $token = trim($CMSNT->site('token_bank'));
    ...
    if ($CMSNT->site('type_bank') == 'VietinBank') {
        $result = curl_get("https://api.sieuthicode.net/historyapiviettinv2/" . $token);
    ...
    if ($CMSNT->site('type_bank') == 'Techcombank') {
        $result = curl_get("https://api.sieuthicode.net/historyapitcbv2/" . $token);
        $result = json_decode($result, true);
        foreach ($result["transactions"] as $data) {
            $tid = check_string($data["transactionID"]);
            $description = check_string($data["description"]);
    ...
    if ($CMSNT->site('type_bank') == 'VPBank') {
        $result = curl_get("https://api.sieuthicode.net/historyapivpbv2/" . $token);
        $result = json_decode($result, true);
        foreach ($result["transactions"] as $data) {
            $tid = check_string($data["transactionID"]);
            $description = check_string($data["description"]);
    ...
    if ($CMSNT->site('type_bank') == 'ACB') {
        $result = curl_get("https://api.sieuthicode.net/historyapiacbv2/" . $token);
        $result = json_decode($result, true);
        foreach ($result["transactions"] as $data) {
            $tid = check_string($data["transactionID"]);
            $description = check_string($data["description"]);
    ...
    if ($CMSNT->site('type_bank') == 'MBBank') {
        $result = curl_get("https://api.sieuthicode.net/historyapimbbankv2/" . $token);
        $result = json_decode($result, true);
        foreach ($result["transactions"] as $data) {
            $tid = check_string($data["transactionID"]);
            $description = check_string($data["description"]);
    ...
    if ($CMSNT->site('type_bank') == 'TPBank') {
        $result = curl_get("https://api.sieuthicode.net/historyapitpbv2/" . $token);
        $result = json_decode($result, true);
        foreach ($result["transactions"] as $data) {
            $tid = check_string($data["transactionID"]);
            $description = check_string($data["description"]);
    ...
    if ($CMSNT->site('type_bank') == 'BIDV') {
        $result = curl_get("https://api.sieuthicode.net/historyapibidvv2/" . $token);
        $result = json_decode($result, true);
        foreach ($result["transactions"] as $data) {
            $tid = check_string($data["transactionID"]);
            $description = check_string($data["description"]);
