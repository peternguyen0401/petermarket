                        ...
                        <div class="card-header">
                            <h3 class="card-title">
                                DANH SÁCH TÀI KHOẢN ĐÃ BÁN <span
                                    class="badge bg-danger"><?=$CMSNT->get_row("SELECT COUNT(id) FROM `accounts` WHERE `product_id` = '".$row['id']."' AND `buyer` IS NOT NULL ")['COUNT(id)'];?></span>
                            </h3>
                            <div class="card-tools">
                                <button type="button" class="btn bg-success btn-sm" data-card-widget="collapse">
                                    <i class="fas fa-minus"></i>
...
<?php foreach ($CMSNT->get_list(" SELECT * FROM `accounts` WHERE `product_id` = '".$row['id']."' AND `buyer` IS NOT NULL ") as $die) { ?>
<?=$die['account'];?>
